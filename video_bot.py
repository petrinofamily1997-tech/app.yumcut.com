...
async def generate_audio(text):
    ...
    try:
        ...
        cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(list_//list_file), "-c", "copy", str(final_audio_path)]
        # Исправленный путь к list_file
        cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(list_file), "-c", "copy", str(final_audio_path)]
        subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        for tf in temp_files: os.remove(tf)
        if list_file.exists(): os.remove(list_file)
        return final_audio_path
    except Exception as e:
        print(f"⚠️ Audio error, using backup: {e}")
        communicate = edge_tts.Communicate(text, "ru-RU-SvetlanaNeural")
        await communicate.save(str(final_audio_path))
        return final_audio_path

def transcribe_audio(audio_path):
    print("👂 Whisper is analyzing audio...")
...
def create_pro_video(scenes, audio_path, words):
    ...
    for scene in scenes:
        ...
        if v_path:
            clip = VideoFileClip(str(v_path)).resize(height=HEIGHT).crop(x_center=CLIP_WIDTH/2 if 'CLIP_WIDTH' in locals() else 540, width=WIDTH)
            clip = clip.loop(duration=scene_duration).set_fps(FPS)
        else:
            clip = ColorClip(size=(WIDTH, HEIGHT), color=(20, 20, 20)).set_duration(scene_duration).set_fps(FPS)
        bg_clips.append(clip)
    
    background = concatenate_videoclips(bg_clips, method="compose").set_audio(full_audio).set_fps(FPS)
    ...
    for w in words:
        try:
            txt = TextClip(
                w['word'].upper(), 
                fontsize=90, color='yellow', font='Arial-Bold', 
                stroke_color='black', stroke_width=3,
                method='caption', size=(WIDTH*0.7, None)
            ).set_start(w['start']).set_end(w['end']).set_position('center').set_fps(FPS)
            subtitle_clips.append(txt)
        except: continue

    final_main = CompositeVideoClip([background] + subtitle_clips).set_fps(FPS)
    ...
    try:
        ...
        outro = ImageClip(str(outro_img_path)).set_duration(3).set_fps(FPS).resize(height=HEIGHT).set_position("center")
        final_video = concatenate_videoclips([final_main, outro], method="compose").set_fps(FPS)
    except:
        final_video = final_main

    output_path = OUTPUT_DIR / "video.mp4"
    # Использование preset='ultrafast' гарантирует правильное закрытие файла
    final_video.write_videofile(str(output_path), fps=FPS, codec="libx264", audio_codec="aac", preset='ultrafast', threads=4)
    
    print(f"✅ Final file size: {output_path.stat().st_size / (1024*1024):.2f} MB")
    return output_path

def choose_topic():
    return random.choice(["Как инфляция в 2026 году съедает сбережения", "Криптовалюты: новый пузырь или будущее денег?", "Как эмоции мешают зарабатывать на бирже", "Почему богатые инвестируют, а бедные копят"])
